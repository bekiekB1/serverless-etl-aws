...
def test_get_unprocessed_files(processor):
    ...
    mock_paginator.paginate.return_value = [{"Contents": [{"Key": "file1.parquet"}, {"Key": "file2.parquet"}]}]
...
def test_mark_as_processed(processor):
    ...
    assert any(tag["Key"] == "ProcessingStatus" and tag["Value"] == "Processed" for tag in tags)
...
def test_lambda_handler_get_unprocessed():
    event = {"action": "get_unprocessed", "bucket": "test-bucket", "prefix": "test/"}
