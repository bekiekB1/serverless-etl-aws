...
from src.lambda_functions.data_downloader import download_and_upload_to_s3, lambda_handler
...
def test_download_and_upload_to_s3():
    ...
    with patch('requests.get', return_value=mock_response) as mock_get, \
        ...
        result = download_and_upload_to_s3(
            'http://test.com/data.parquet',
            'test-bucket',
            '2024-01'
        )
...
def test_lambda_handler_success(mock_env):
    event = {
        'url': 'http://test.com/data.parquet',
        'year_month': '2024-01'
    }
